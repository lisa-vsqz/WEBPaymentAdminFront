...
import {
  ...
  createProviderHistory,
  updateProviderHistory,
  deleteProviderHistory,
} from "../../../services/providerHistoryService";
...
export default function ListProviderHistory() {
  ...
  const handleDelete = async (id) => {
    await deleteProviderHistory(id);
    fetchProviderHistory();
  };
  const handleSave = async (historyData) => {
    formView === "create"
      ? await createProviderHistory(historyData)
      : await updateProviderHistory(
          selectedHistory.ProviderHistoryID,
          historyData
        );
    fetchProviderHistory();
    setFormView(null);
    ...
    <div className="container mx-auto p-4">
      ...
      {!formView ? (
        <EntityTable
          ...
          columns={providerHistoryColumns}
          onRead={handleRead}
